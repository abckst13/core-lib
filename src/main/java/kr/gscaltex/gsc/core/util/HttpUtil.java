...
public class HttpUtil {
	...
	/**
	 * request url에 있는 domain 정보만 분리하여 가져온다.
	 * @param request
	 * @return
	 * @throws Exception
	 */
	public static String getRequestDomain(HttpServletRequest request) throws Exception {
		return request.getRequestURL().toString().replace(request.getRequestURI(), "").replace("?"+request.getQueryString(), "");
	}
	 ...
	 * @throws Exception
	 */
	public static String getRequestDomain() throws Exception {
		return getRequestDomain(SpringUtil.getHttpServletRequest());
	}

	/**
	 * Get MediaType of HttpMessage
	 *
	 * @param httpInputMessage wrapping of HttpServletRequest / HttpServletResponse
	 * @return {@link MediaType}
	 */
	public static MediaType getMediaType(HttpInputMessage httpInputMessage) {
		return MediaType.valueOf(Optional.ofNullable(httpInputMessage.getHeaders().get("Content-Type")).orElse(new ArrayList<String>()).stream().findFirst().orElse("text/plain"));
	}

	/**
	 * Checks if is application/json
	 *
	 * @param httpInputMessage wrapping of HttpServletRequest / HttpServletResponse
	 * @return true, if is application/json
	 */
	public static boolean isApplicationJson(HttpInputMessage httpInputMessage) {
		return MediaType.APPLICATION_JSON.isCompatibleWith(getMediaType(httpInputMessage));
	}
	...
	public static String getRemoteIpAddr() {
		return getRemoteIpAddr(SpringUtil.getHttpServletRequest());
	}
	...
	public static String getRemoteIpAddr(HttpServletRequest request) {
		...
		return HttpUtil.getRemoteAddr(request).replaceAll(CmnConst.LOCAL_IP6, CmnConst.LOCAL_IP);
	}
	 ...
	 * @return the header to box
	 */
	public static Box getHeaderToBox(HttpServletResponse response) {
...
//		if(uri.endsWith(".json") || uri.endsWith(".xml")) {
	...
	public static final String getUrlEncode(String str) {
		...
		try {
			encodeStr = URLEncoder.encode(str, CmnConst.DEFAULT_CHARSET);
	...
	public static final String getUrlDecode(String str) {
		String decodeStr = null;
		try {
			decodeStr = URLDecoder.decode(str, CmnConst.DEFAULT_CHARSET);
		} catch (UnsupportedEncodingException e) {
			log.error("error : {}", e.getMessage());
		}
