package kr.gscaltex.gsc.core.data;
...
import kr.gscaltex.gsc.core.util.BoxUtil;
import kr.gscaltex.gsc.core.util.NumberUtil;
import kr.gscaltex.gsc.core.util.StringUtil;
import kr.gscaltex.gsc.core.util.paginate.PaginateModel;
...
//	private HttpSession session;
	...
	/**
	 * request의 파라미터를 box에 담습니다. (restApi 연동시 body에 json string방식으로 연동하기때문에 deprecated)
	 *
	 * @param request
	 * @throws Exception
	 */
...
//		this.put("refererUri", HttpUtil.getRefererUri(request));
	 ...
	 * @return the path int
	 */
	public int getPathInt(String path, int defaultValue) {
		if(jxPathContext == null) {
			jxPathContext = JXPathContext.newContext(this);
		}
		...
		int rtnValue = 0;
		try {
			rtnValue = NumberUtil.toInt(jxPathContext.getValue(path));
		} catch (Exception e) {
			rtnValue = defaultValue;
		}
		return rtnValue;
