...
public class SMSComponent {
	...
	// 일반적으로 사용하게될 변수입니다.
		...
		// 실제 발송하는 과정입니다.
		public Box SendMsg(String[] strDestList, String strData , String strSubject ,int nCount) throws Exception
		{
			Box resBox = new Box();
			int i = 0;
			String result = "", msgid = "";
			String strCallBack = SMSConfig.ICODE_SEND_NUM;
			// 발송내역 유니코드로 변환.
			Random random = new Random();
			int randomNum = random.nextInt(899999) + 100000;
			String strNum = Integer.toString(randomNum);
			strData = "[AI-PEPPERS] 본인확인 인증번호 [" + strNum +"]"+ "입니다.";
			resBox.put("accessCd", strNum);
