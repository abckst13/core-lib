...
public class KisaSeed {
	...
	public static int SEED_CBC_Process(KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out, int[] outLen) {
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			Common.memcpy(pInfo.buffer, in, in_offset, pInfo.buffer_length);
		} else {
			pdwXOR = pInfo.ivec;
			in_offset = 0;
			out_offset = 0;
			...
			Common.memcpy(pInfo.ivec, pdwXOR, pdwXOR_offset, BLOCK_SIZE_SEED);
			Common.memcpy(pInfo.last_block, out, out_offset - BLOCK_SIZE_SEED_INT, BLOCK_SIZE_SEED);
	...
	public static int SEED_CBC_Close(KISA_SEED_INFO pInfo, int[] out, int out_offset, int[] outLen) {
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			for (i = pInfo.buffer_length; i < BLOCK_SIZE_SEED; i++) {
				Common.set_byte_for_int(pInfo.buffer, i, (byte) nPaddngLeng, ENDIAN);
			}
			BLOCK_XOR_CBC(pInfo.buffer, 0, pInfo.buffer, 0, pInfo.ivec, 0);

			KISA_SEED_Encrypt_Block_forCBC(pInfo.buffer, 0, out, out_offset, pInfo.seed_key);
			...
			nPaddngLeng = Common.get_byte_for_int(pInfo.last_block, BLOCK_SIZE_SEED - 1, ENDIAN);
	...
	public static final class KISA_SEED_INFO {
		...
		public int[] last_block = new int[4];
		...
		public KISA_SEED_INFO() {
			...
			buffer_length = 0;
			last_block[0] = last_block[1] = last_block[2] = last_block[3] = 0;
			last_block_flag = 0;
		}
	...
	public static int SEED_ECB_Process(KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out, int[] outLen) {
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			outLen[0] = nCurrentCount - BLOCK_SIZE_SEED;
			Common.memcpy(pInfo.last_block, out, out_offset - 4, BLOCK_SIZE_SEED);
		}
	...
	public static int SEED_ECB_Close(KISA_SEED_INFO pInfo, int[] out, int out_offset, int[] outLen) {
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt) {
			...
			KISA_SEED_Encrypt_Block_forECB(pInfo.buffer, 0, out, (out_offset) / 4, pInfo.seed_key);
			outLen[0] = BLOCK_SIZE_SEED;
		} else {
			nPaddngLeng = Common.get_byte_for_int(pInfo.last_block, BLOCK_SIZE_SEED - 1, ENDIAN);
			if (nPaddngLeng > 0 && nPaddngLeng <= BLOCK_SIZE_SEED) {
	...
	private static void BLOCK_XOR_SEED(int[] OUT_VALUE, int out_value_offset, int[] IN_VALUE1, int in_value1_offset,
			int[] IN_VALUE2, int in_value2_offset) {
		OUT_VALUE[out_value_offset + 0] = (in_value1_offset < IN_VALUE1.length ? IN_VALUE1[in_value1_offset + 0] : 0)
				^ (in_value2_offset < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 0] : 0);
		OUT_VALUE[out_value_offset
	...
	private static void KISA_SEED_Encrypt_Block_forCTR(int[] in, int in_offset, int[] out, int out_offset,
			KISA_SEED_KEY ks) {
		int LR[] = new int[4]; // Iuput/output values at each rounds
		int T[] = new int[2]; // Temporary variables for round function F
		int K[] = ks.key_data; // Pointer of round keys
	...
	public static int SEED_CTR_init(KISA_SEED_INFO pInfo, KISA_ENC_DEC enc, byte[] pszUserKey, byte[] pbszCTR) {
		...
		if (null == pInfo || null == pszUserKey || null == pbszCTR)
			return 0;
	...
	public static int SEED_CTR_Process(KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out, int[] outLen) {
		...
		while (nCurrentCount < inLen) {
			...
			UpdateCounter_for_SEED(pdwCounter, pbszCounter_offset, 1, (BLOCK_SIZE_SEED - 1));
	...
	public static byte[] SEED_CTR_Encrypt(byte[] pbszUserKey, byte[] pbszCTR, byte[] message, int message_offset,
			int message_length) {
		int nOutLeng[] = { 0 };
		int nPaddingLeng[] = new int[] { 0 };
		KISA_SEED_INFO info = new KISA_SEED_INFO();
		...
		int nInputTextPadding = (BLOCK_SIZE_SEED - (message_length % BLOCK_SIZE_SEED)) % BLOCK_SIZE_SEED;
		...
		outlen = ((newmessage.length / BLOCK_SIZE_SEED)) * BLOCK_SIZE_SEED_INT;
		outbuf = new int[outlen];
	...
	public static byte[] SEED_CTR_Decrypt(byte[] pbszUserKey, byte[] pbszCTR, byte[] message, int message_offset,
			int message_length) {
		int nOutLeng[] = { 0 };
		int nPaddingLeng[] = new int[] { 0 };
		KISA_SEED_INFO info = new KISA_SEED_INFO();
